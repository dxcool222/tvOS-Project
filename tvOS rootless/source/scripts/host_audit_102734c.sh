...
IPA="${1:?usage: host_audit_102734c.sh <ipa-path>}"
LDID="${LDID:-/opt/local/bin/ldid}"

EXPECTED_HOOK_SHA="8446a2386197a7fd0a13e7701843f7c29d8682b4d6fd759909be25c87bf7f0e4"
...
MANIFEST="$H516/BUILD102734C_RESOURCE_MANIFEST.txt"
...
if [[ -f "$MANIFEST" ]] \
    && rg -q "^CFBundleVersion=102734$" "$MANIFEST" \
...
if [[ -x "$LDID" ]] \
    ...
    && "$LDID" -e "$OPAINJECT" | rg -q "platform-application"; then
    helper_entitlements=PASS
fi

app_markers=FAIL
if rg -q "BUILD102734C_SCOPE=CONSTRUCTOR_BOOMERANG_ONLY_OPAINJECT_TASKPORT_REPAIR" "$APP_STRINGS" \
    && rg -q "BUILD102734C_BUNDLE_RESOURCE_IDENTITY" "$APP_STRINGS" \
    && rg -q "BUILD102734C_BUNDLE_FILE_SHA_POLICY" "$APP_STRINGS" \
    && rg -q "BUILD102734C_STAGE_COPY_IDENTITY" "$APP_STRINGS" \
    && rg -q "BUILD102734C_STAGE_COPY_COMPARE" "$APP_STRINGS" \
    && rg -q "BUILD102734C_TRUSTCACHE_ENTRY_COUNT" "$APP_STRINGS" \
    && rg -q "BUILD102734C_OPAINJECT_WAIT_STATUS" "$APP_STRINGS" \
    && rg -q "BUILD102734C_OPAINJECT_EXIT_CODE" "$APP_STRINGS"; then
    app_markers=PASS
fi

helper_markers=FAIL
if rg -q "BUILD102734C_OPAINJECT_TARGET_PID" "$HELPER_STRINGS" \
    && rg -q "BUILD102734C_TASK_FOR_PID_RC" "$HELPER_STRINGS" \
    && rg -q "BUILD102734C_TASK_PORT_VALID" "$HELPER_STRINGS" \
    && rg -q "BUILD102734C_REMOTE_DLOPEN_ATTEMPTED" "$HELPER_STRINGS" \
    && rg -q "BUILD102734C_OPAINJECT_FAILURE_BRANCH" "$HELPER_STRINGS"; then
    helper_markers=PASS
fi
...
host=PASS
[[ "$CFBUNDLEVERSION" == "102734" ]] || host=FAIL
[[ "$manifest_result" == PASS ]] || host=FAIL
[[ "$trio_hashes" == PASS ]] || host=FAIL
...
cat <<EOF
BUILD102734C_HOST_AUDIT_REPORT
IPA=$IPA
...
FROZEN_TRIO_HASHES=$trio_hashes
...
APP_102734_MARKERS=$app_markers
HELPER_102734_MARKERS=$helper_markers
