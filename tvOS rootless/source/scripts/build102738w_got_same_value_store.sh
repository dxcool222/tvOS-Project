...
BUILD_ROOT="${DT_BUILD_OUTPUT_ROOT:-$PROJECT/build/102738W}"
...
HOOK_CFLAGS=(
  ...
  -DDT_BUILD102738W_TELEMETRY=1
  "${INC[@]}"
)

echo "=== BUILD102738W launchd GOT same-value-store hook rebuild ==="
...
"${CC[@]}" "${HOOK_CFLAGS[@]}" -c "$MIN/dt_launchdhook516_got_store_102738w.c" -o "$OBJ/got_store.o"
...
"${CC[@]}" -arch arm64 -isysroot "$SDK" -mtvos-version-min="$TVOS_MIN" \
  ...
  -L"$OUT" -ljailbreak \
  "$OBJ/main_gate1b.o" "$OBJ/boomerang.o" "$OBJ/got_store.o" \
...

{
    ...
    echo "SCOPE=LAUNCHD_GOT_SAME_VALUE_STORE_ONLY"
    echo "VARIANT=102738W_SAME_VALUE_GOT_STORE"
...
rg -q 'BUILD102738W_PROBE_ENTER' "$HOOK_STRINGS"
rg -q 'BUILD102738W_SAME_VALUE_STORE_IMPLEMENTED_YES' "$HOOK_STRINGS"
...
rg -q 'GOT_PRESTORE_MATCH_PASS' "$HOOK_STRINGS"
rg -q 'GOT_SAME_VALUE_STORE_PASS' "$HOOK_STRINGS"
rg -q 'GOT_SAME_VALUE_STORE_READBACK' "$HOOK_STRINGS"
rg -q 'GOT_POINTER_REPLACED_NO' "$HOOK_STRINGS"
if rg -q 'MSHookFunction|initXPCHooks|GOT_POINTER_REPLACED_YES|xpc_receive_mach_msg_hook' "$HOOK_STRINGS"; then
    echo "ERROR: functional hook backend or replacement pointer leaked into same-value-store build" >&2
    exit 1
fi
nm -u "$HOOK" 2>/dev/null | rg -q '_vm_region_recurse_64'
nm "$HOOK" 2>/dev/null | rg -q '_dt102738w_run_got_same_value_store_probe'
nm "$HOOK" 2>/dev/null | rg -q '_dt102738w_atomic_store_same_value'

echo "BUILD102738W_HOOK_ONLY_REBUILD=PASS"
echo "BUILD102738R_POINTER_VALIDATOR_REPAIR=PASS"
echo "BUILD102738W_FROZEN_RESOURCES=PASS"
echo "BUILD102738W_HOOK_SHA256=$(shasum -a 256 "$HOOK" | awk '{print $1}')"
echo "BUILD102738W_HANDOFF_LIBJAILBREAK_SHA256=$(shasum -a 256 "$OUT/libjailbreak.dylib" | awk '{print $1}')"
echo "BUILD102738W_APP_FRAMEWORK_LIBJAILBREAK_SHA256=$(shasum -a 256 "$FW_OUT/libjailbreak.dylib" | awk '{print $1}')"
