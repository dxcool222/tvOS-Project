...
MAKE_PROJECT="/tmp/dopamin_tvos_kfd_102739k_obs1_src"
TEMP_IPA="dopamin-tvOS-kfd-102738P-LAUNCHD-GOT-PROTECTION-ONLY.ipa"
IPA_NAME="dopamin-tvOS-kfd-102739K-OBS1-CONSOLE-VISIBLE-READ-ONLY-PREFLIGHT.ipa"
BUILD_SLOT="102739K_OBS1"
OUTPUT_DIR="$REPO_ROOT/Output"
ARCHIVE="$REPO_ROOT/tvOS/tvbootstrap-ssh-1900.tar.zst"
FROZEN_J="$PROJECT/build/102739J/Handoff516"

export CLANG_MODULE_CACHE_PATH="${CLANG_MODULE_CACHE_PATH:-/tmp/dt102739k_obs1_module_cache}"
rm -rf "$CLANG_MODULE_CACHE_PATH" "$MAKE_PROJECT"
mkdir -p "$CLANG_MODULE_CACHE_PATH" "$MAKE_PROJECT"

echo "=== BUILD102739K-OBS1 verify frozen J and pinned bootstrap inputs ==="
...
echo "=== BUILD102739K-OBS1 regenerate unchanged auxiliary helpers ==="
...
echo "=== BUILD102739K-OBS1 compile and package on repaired K flags ==="
...
DT_EXPECT_102738_VARIANT=K \
    ...
    | tee "$PROJECT/docs/reports/BUILD_ARTIFACT_IDENTITY_102739K_OBS1.txt"
...
{
    echo "BUILD102739K_OBS1_FINAL_HOST_AUDIT"
...
} | tee "$PROJECT/docs/reports/BUILD102739K_OBS1_FINAL_HOST_AUDIT.txt"

mkdir -p "$OUTPUT_DIR"
cp "$PROJECT/$IPA_NAME" "$OUTPUT_DIR/$IPA_NAME"
echo "BUILD102739K_OBS1_PACKAGE_COMPLETE=YES"
echo "BUILD102739K_OBS1_IPA_PATH=$OUTPUT_DIR/$IPA_NAME"
echo "BUILD102739K_OBS1_IPA_SHA256=$IPA_SHA"
