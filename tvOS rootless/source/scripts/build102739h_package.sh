...
MAKE_PROJECT="/tmp/dopamin_tvos_kfd_102739h_src"
TEMP_IPA="dopamin-tvOS-kfd-102738P-LAUNCHD-GOT-PROTECTION-ONLY.ipa"
IPA_NAME="dopamin-tvOS-kfd-102739H-READ-ONLY-ACTION-ARGUMENT-MARSHALLING.ipa"

export CLANG_MODULE_CACHE_PATH="${CLANG_MODULE_CACHE_PATH:-/tmp/dt102739h_module_cache}"
rm -rf "$CLANG_MODULE_CACHE_PATH" "$MAKE_PROJECT"
mkdir -p "$CLANG_MODULE_CACHE_PATH" "$MAKE_PROJECT"

echo "=== BUILD102739H prepare isolated source copy ==="
rsync -a --delete "$PROJECT/" "$MAKE_PROJECT/"
rm -rf "$MAKE_PROJECT/.theos/obj/appletv" "$MAKE_PROJECT/.theos/build_session"

echo "=== BUILD102739H build read-only argument marshalling hook and observer ==="
DT_BUILD102739H_MODE=1 \
...
echo "=== BUILD102739H regenerate auxiliary helpers ==="
...
echo "=== BUILD102739H compile and package on functional gate 102738 ==="
DT_WORKSPACE_ROOT="$REPO_ROOT" DT_102739H_VARIANT=1 DT_102738_PREBUILT=1 \
  make -C "$MAKE_PROJECT" ipa

[[ -f "$MAKE_PROJECT/$TEMP_IPA" ]]
rm -rf "$PROJECT/build/102739H"
cp -R "$MAKE_PROJECT/build/102738P" "$PROJECT/build/102739H"
rm -f "$PROJECT/build/102739H/$TEMP_IPA"
cp "$MAKE_PROJECT/$TEMP_IPA" "$PROJECT/build/102739H/$IPA_NAME"
cp "$MAKE_PROJECT/$TEMP_IPA" "$PROJECT/$IPA_NAME"

DT_EXPECT_102738_VARIANT=H \
  bash "$PROJECT/scripts/verify_build_artifact_identity.sh" "$PROJECT/$IPA_NAME" \
  | tee "$PROJECT/docs/reports/BUILD_ARTIFACT_IDENTITY_102739H.txt"

HOOK="$PROJECT/build/102739H/Handoff516/launchdhook516.dylib"
HELPER="$PROJECT/build/102739H/Handoff516/dt_opainject516"
OBJ="$PROJECT/build/102739H/obj"
...
RESOURCE_MANIFEST="$PROJECT/build/102739H/Handoff516/BUILD102739H_RESOURCE_MANIFEST.txt"
...
if rg -q '_xpc_(release|retain|dictionary_create_reply)|_fprintf|_open|_mmap|_write|_jbserver|_jbinfo|_strdup|_malloc|_free' \
    "$WRAPPER_DISASM"; then
    echo "ERROR: forbidden ownership, reply, allocation, I/O, jbinfo, or jbserver call in 102739H wrapper" >&2
    exit 1
fi
rg -q 'bl.*dt102739g_unreachable_get_jbroot' "$WRAPPER_DISASM" && {
    echo "ERROR: GET_JBROOT placeholder is reachable from 102739H wrapper" >&2
    exit 1
}
...
for marker in \
    ...
    BUILD102739H_ARG_DESCRIPTOR_COUNT= \
...
for marker in \
    BUILD102739H_SCOPE=READ_ONLY_ACTION_ARGUMENT_MARSHALLING \
    BUILD102739H_BLUEPRINT=IOS_JBSERVER_LINES_59_THROUGH_102 \
    BUILD102739H_TRIGGER_REQUEST_COUNT=1 \
    BUILD102739H_TRIGGER_DOMAIN=1 \
    BUILD102739H_TRIGGER_ACTION=1 \
    BUILD102739H_ARGUMENT_DESCRIPTOR_COUNT=1 \
    BUILD102739H_ARGUMENT_DESCRIPTOR_NAME=root-path \
    BUILD102739H_ARGUMENT_DESCRIPTOR_TYPE=JBS_TYPE_STRING \
    BUILD102739H_ARGUMENT_DESCRIPTOR_DIRECTION=OUT \
    BUILD102739H_HANDLER_INVOCATION_IMPLEMENTED=NO \
    BUILD102739H_REPLY_CREATION_IMPLEMENTED=NO \
    BUILD102739H_BOOTSTRAP_CHANGED=NO; do
...
for frozen in \
    ...
    file="${frozen%%:*}"
    expected="${frozen#*:}"
    actual="$(shasum -a 256 "$PROJECT/build/102739H/Handoff516/$file" | awk '{print $1}')"
    [[ "$actual" == "$expected" ]]
done

[[ -f "$RESOURCE_MANIFEST" ]]
rg -q '^ARGUMENT_DESCRIPTOR_COUNT=1$' "$RESOURCE_MANIFEST"
rg -q '^ARGUMENT_DESCRIPTOR_NAME=root-path$' "$RESOURCE_MANIFEST"
rg -q '^ARGUMENT_DESCRIPTOR_TYPE=JBS_TYPE_STRING$' "$RESOURCE_MANIFEST"
rg -q '^ARGUMENT_DESCRIPTOR_DIRECTION=OUT$' "$RESOURCE_MANIFEST"
rg -q '^OUTPUT_SLOT_BINDING_ENABLED=YES$' "$RESOURCE_MANIFEST"
...
{
    echo "BUILD102739H_FINAL_HOST_AUDIT"
    ...
    echo "IOS_DISPATCH_PREFIX=type_keys_domain_token_permission_action_handler_pointer_args_argsout_descriptor_bind"
    echo "LAUNCHD_GOT_OFFSET=0x65018"
    echo "ORIGINAL_CALL_COUNT=1"
    echo "PERMISSION_HANDLER_CALL_COUNT=1"
    echo "ACTION_HANDLER_CALL_COUNT=0"
...
} | tee "$PROJECT/docs/reports/BUILD102739H_FINAL_HOST_AUDIT.txt"

echo "BUILD102739H_PACKAGE_COMPLETE=YES"
echo "BUILD102739H_IPA_PATH=$PROJECT/$IPA_NAME"
echo "BUILD102739H_IPA_SHA256=$(shasum -a 256 "$PROJECT/$IPA_NAME" | awk '{print $1}')"
