...
MAKE_PROJECT="/tmp/dopamin_tvos_kfd_102739k_src"
TEMP_IPA="dopamin-tvOS-kfd-102738P-LAUNCHD-GOT-PROTECTION-ONLY.ipa"
IPA_NAME="dopamin-tvOS-kfd-102739K-ROOTFUL-BOOTSTRAP-READ-ONLY-PREFLIGHT.ipa"
OUTPUT_DIR="$REPO_ROOT/Output"
ARCHIVE="$REPO_ROOT/tvOS/tvbootstrap-ssh-1900.tar.zst"
FROZEN_J="$PROJECT/build/102739J/Handoff516"

export CLANG_MODULE_CACHE_PATH="${CLANG_MODULE_CACHE_PATH:-/tmp/dt102739k_module_cache}"
rm -rf "$CLANG_MODULE_CACHE_PATH" "$MAKE_PROJECT"
mkdir -p "$CLANG_MODULE_CACHE_PATH" "$MAKE_PROJECT"

echo "=== BUILD102739K verify frozen J and pinned bootstrap inputs ==="
...
echo "=== BUILD102739K regenerate unchanged auxiliary helpers ==="
...
echo "=== BUILD102739K compile and package on frozen J descendant flags ==="
DT_WORKSPACE_ROOT="$REPO_ROOT" DT_102739K_VARIANT=1 DT_102738_PREBUILT=1 \
    make -C "$MAKE_PROJECT" ipa

[[ -f "$MAKE_PROJECT/$TEMP_IPA" ]]
rm -rf "$PROJECT/build/102739K"
cp -R "$MAKE_PROJECT/build/102738P" "$PROJECT/build/102739K"
rm -f "$PROJECT/build/102739K/$TEMP_IPA"
cp "$MAKE_PROJECT/$TEMP_IPA" "$PROJECT/build/102739K/$IPA_NAME"
...
DT_EXPECT_102738_VARIANT=K \
    ...
    | tee "$PROJECT/docs/reports/BUILD_ARTIFACT_IDENTITY_102739K.txt"

APP_BINARY="$PROJECT/build/102739K/obj/app_binary"
APP_STRINGS="$PROJECT/build/102739K/obj/app_strings.txt"
PREFLIGHT_DISASM="$PROJECT/build/102739K/obj/preflight_disassembly.txt"
APP_FLOW_DISASM="$PROJECT/build/102739K/obj/wall2_restore_order_disassembly.txt"
ACTIVE_WINDOW_DISASM="$PROJECT/build/102739K/obj/wall2_post_inject_active_window_disassembly.txt"
PACKAGED_MANIFEST="$PROJECT/build/102739K/obj/BUILD102739K_CF1900_PATHS.tsv"
SOURCE_MANIFEST="$MAKE_PROJECT/bootstrap_preflight/BUILD102739K_CF1900_PATHS.tsv"
mkdir -p "$PROJECT/build/102739K/obj"
...
for call in _lstat _statfs _access _sysctlbyname _geteuid \
...
for frozen_file in launchdhook516.dylib dt_opainject516 dt_jbctl516 \
    ...
    cmp -s "$FROZEN_J/$frozen_file" "$PROJECT/build/102739K/Handoff516/$frozen_file"
done

HOOK_SHA="$(shasum -a 256 "$PROJECT/build/102739K/Handoff516/launchdhook516.dylib" | awk '{print $1}')"
HELPER_SHA="$(shasum -a 256 "$PROJECT/build/102739K/Handoff516/dt_opainject516" | awk '{print $1}')"
MANIFEST_SHA="$(shasum -a 256 "$PACKAGED_MANIFEST" | awk '{print $1}')"
IPA_SHA="$(shasum -a 256 "$PROJECT/$IPA_NAME" | awk '{print $1}')"

{
    echo "BUILD102739K_FINAL_HOST_AUDIT"
    echo "TARGET_MODEL=AppleTV6,2"
    echo "TARGET_TVOS_VERSION=16.5"
    echo "TARGET_BUILD=20L563"
    echo "BASELINE=BUILD102739J_FROZEN"
    echo "SCOPE=ROOTFUL_BOOTSTRAP_READ_ONLY_PREFLIGHT"
...
} | tee "$PROJECT/docs/reports/BUILD102739K_FINAL_HOST_AUDIT.txt"

mkdir -p "$OUTPUT_DIR"
cp "$PROJECT/$IPA_NAME" "$OUTPUT_DIR/$IPA_NAME"
echo "BUILD102739K_PACKAGE_COMPLETE=YES"
echo "BUILD102739K_IPA_PATH=$OUTPUT_DIR/$IPA_NAME"
echo "BUILD102739K_IPA_SHA256=$IPA_SHA"
