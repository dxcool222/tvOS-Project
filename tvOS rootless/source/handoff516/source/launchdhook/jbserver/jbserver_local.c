...
typedef mach_msg_return_t (*dt_mach_msg_send_fn)(mach_msg_header_t *);

static mach_msg_return_t dt_mach_msg_send(mach_msg_header_t *hdr)
{
	static dt_mach_msg_send_fn fn;
	if (!fn)
		fn = (dt_mach_msg_send_fn)dlsym(RTLD_DEFAULT, "mach_msg_send");
	return fn ? fn(hdr) : (mach_msg_return_t)-1;
}
