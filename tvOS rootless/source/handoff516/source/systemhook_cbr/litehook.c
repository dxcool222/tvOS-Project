...
kern_return_t litehook_hook_function(void *source, void *target)
{
	...
	return litehook_verify_function_hook(source, target) ? KERN_SUCCESS : KERN_FAILURE;
}

bool litehook_verify_function_hook(void *source, void *target)
{
	if (!source || !target) return false;
	uint32_t *toHook = (uint32_t *)ptrauth_strip(source, ptrauth_key_function_pointer);
	uint64_t targetAddr = (uint64_t)ptrauth_strip(target, ptrauth_key_function_pointer);
	uint32_t expected[5] = {
		_lth_arm64_gen_movk(16, targetAddr >> 0, 0),
		_lth_arm64_gen_movk(16, targetAddr >> 16, 16),
		_lth_arm64_gen_movk(16, targetAddr >> 32, 32),
		_lth_arm64_gen_movk(16, targetAddr >> 48, 48),
		_lth_arm64_gen_br(16),
	};
	return memcmp(toHook, expected, sizeof(expected)) == 0;
}
...
size_t _litehook_rebind_symbol_in_section(const mach_header_u *targetHeader, section_u *section, void *replacee, void *replacement)
{
	...
	if (!symbolPointers || sectionSize == 0) return 0;
	size_t replaced = 0;
...
size_t _litehook_apply_global_rebind(const mach_header_u *mh, global_rebind *rebind)
{
	if (mh != rebind->sourceHeader) {
		bool filterAllowed = true;
		if (rebind->exceptionFilter) filterAllowed = rebind->exceptionFilter(mh);
		if (filterAllowed) {
			return litehook_rebind_symbol(mh, rebind->replacee, rebind->replacement, NULL);
		}
	}
	return 0;
}
...
void _litehook_apply_global_rebinds(const mach_header_u *mh, intptr_t vmaddr_slide)
{
	...
	for (uint32_t i = 0; i < gRebindCount; i++) {
		...
		(void)_litehook_apply_global_rebind(mh, &gRebinds[i]);
	}
}

size_t litehook_rebind_symbol(const mach_header_u *targetHeader, void *replacee, void *replacement, bool (*exceptionFilter)(const mach_header_u *header))
{
	size_t replaced = 0;
	if (targetHeader == LITEHOOK_REBIND_GLOBAL) {
		...
		if (dladdr(replacement, &replacementInfo) == 0) return 0;
		if (replacementInfo.dli_fname == NULL) return 0;
		...
		if (!sourceHeader) return 0;

		if (!gRebinds) {
			_dyld_register_func_for_add_image((void (*)(const struct mach_header *, intptr_t))_litehook_apply_global_rebinds);
		}

		global_rebind *newRebinds = realloc(gRebinds,
		    sizeof(global_rebind) * (gRebindCount + 1));
		if (!newRebinds) return 0;
		gRebinds = newRebinds;
		gRebindCount++;
		...
		for (uint32_t i = 0; i < _dyld_image_count(); i++) {
			...
			replaced += _litehook_apply_global_rebind(header, rebind);
		}
	}
	else {
				...
				if (!strncmp(segCmd->segname, "__AUTH_CONST", sizeof(segCmd->segname)) ||
					...
					for (int j = 0; j < segCmd->nsects; j++) {
						if ((sections[j].flags & SECTION_TYPE) == S_LAZY_SYMBOL_POINTERS || 
							...
							replaced += _litehook_rebind_symbol_in_section(targetHeader, &sections[j], replacee, replacement);
						}
					}
				}
			}
