...
struct system_info {
	...
	struct {
		...
		/* tvOS 16.5 / 20L563 DarkSword pre-17 socket→proto path.
		 * Must stay ABI-identical to app compile header (DOP2 info.h inpcb tail).
		 * Missing this tail made DTApplyTvOSInpcbOverrides smash adjacent gPrimitives. */
		struct {
			uint32_t list_next;
			uint32_t list_prev;
			uint32_t pcbinfo;
