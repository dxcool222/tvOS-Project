/*
 * dopamin-tvOS-kfd override: misaka-style find_proc (see dt_misaka_find_proc.c).
 * Remainder matches Dopamine libkfd kread_sem_open.h.
 */

#ifndef kread_sem_open_h
#define kread_sem_open_h

#include "dt_misaka_find_proc.h"
...
void kread_sem_open_init(struct kfd* kfd)
{
    ...
    if (sem_fd <= 0) {
        print_failure("sem_open init failed: errno=%d (%s)", errno, strerror(errno));
        kfd->kread.krkw_op_failed = true;
        return;
    }
    ...
    i32 proc_ret = syscall(SYS_proc_info, callnum, pid, flavor, arg, buffer, buffersize);
    if (proc_ret != buffersize) {
        print_failure("proc_info init failed: ret=%d errno=%d (%s)", proc_ret, errno, strerror(errno));
        close(sem_fd);
        kfd->kread.krkw_op_failed = true;
        return;
    }
...
void kread_sem_open_allocate(struct kfd* kfd, u64 id)
{
    ...
    if (fd <= 0) {
        print_failure("sem_open allocate id=%llu failed: errno=%d (%s)", id, errno, strerror(errno));
        kfd->kread.krkw_op_failed = true;
        return;
    }
...
{
    dt_misaka_kread_sem_open_find_proc(kfd);
}
