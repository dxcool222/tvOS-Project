...
int injectDylibViaRop(task_t task, pid_t pid, const char* dylibPath, vm_address_t allImageInfoAddr, bool skipSandboxFixup)
{
	...
	if (skipSandboxFixup) {
		fprintf(stderr, "KCALL681_SANDBOX_FIXUP_SKIPPED_POST_KCALL\n");
	}
	else {
		if (!sandboxFixup(task, pthread, pid, dylibPath, allImageInfoAddr)) {
			fprintf(stderr, "KCALL681_SANDBOX_FIXUP_REQUIRED_BUT_MISSING\n");
	...
	if(remoteDylibPath)
	{
		...
		if (dlopenRet) {
			...
			fprintf(stderr, "KCALL681_REMOTE_DLOPEN_WORKING\n");
			ret = 0;
		}
		else {
